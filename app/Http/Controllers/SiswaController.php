    ...
    public function destroy(Request $request, $id)
{
    try {
        ...
        if ($siswa->foto_ktp) {
            \Storage::delete('public/' . $siswa->foto_ktp);
        }
        if ($siswa->pas_foto) {
            \Storage::delete('public/' . $siswa->pas_foto);
        }
        if ($siswa->foto_ijazah) {
            \Storage::delete('public/' . $siswa->foto_ijazah);
        }

        // Hapus data siswa
        $siswa->delete();

        // Ambil parameter halaman saat ini dari URL sebelumnya
        $previousUrl = url()->previous();
        $currentPage = $request->input('page', 1); // Dapatkan halaman saat ini atau default ke 1

        // Redirect ke halaman sebelumnya dengan pesan sukses
        return redirect()->to($previousUrl)->with('success', 'Data siswa berhasil dihapus.');
    } catch (\Exception $e) {
